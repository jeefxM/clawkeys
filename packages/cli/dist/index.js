...
var DEFAULT_HD_PATH = "m/44'/60'/0'/0/0";
function generateWallet(options) {
  ...
  if (withMnemonic) {
    ...
    const privateKey2 = `0x${Buffer.from(derivedKey.privateKey).toString("hex")}`;
...
function formatJson(wallet) {
  return JSON.stringify(wallet, null, 2);
}
var program = new Command();
program.name("clawkeys").description("Secure local wallet generation for AI agents").version("1.0.0");
program.command("generate").description("Generate a new EVM wallet locally").option("-m, --mnemonic", "Generate with mnemonic seed phrase", false).option("-w, --words <number>", "Number of mnemonic words (12 or 24)", "12").option("--path <hdpath>", "HD derivation path", DEFAULT_HD_PATH).option("-j, --json", "Output as JSON", false).option("-c, --count <number>", "Number of wallets to generate", "1").action((options) => {
